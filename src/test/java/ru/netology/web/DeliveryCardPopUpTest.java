...
public class DeliveryCardPopUpTest {
    ...
    @Test
    void shouldTestCityList() {
        open("http://localhost:9999");
        $("[data-test-id='city'] input").setValue("Як");
        $$(".menu-item .menu-item__control").find(exactText("Брянск")).click();

        $("[data-test-id='city'] input").doubleClick().sendKeys(Keys.BACK_SPACE);
        $("[data-test-id='city'] input").setValue("Як");
        $$(".menu-item .menu-item__control").find(exactText("Красноярск")).click();

        $("[data-test-id='city'] input").doubleClick().sendKeys(Keys.BACK_SPACE);
        $("[data-test-id='city'] input").setValue("Як");
        $$(".menu-item .menu-item__control").find(exactText("Ульяновск")).click();

        $("[data-test-id='city'] input").doubleClick().sendKeys(Keys.BACK_SPACE);
        $("[data-test-id='city'] input").setValue("Як");
        $$(".menu-item .menu-item__control").find(exactText("Челябинск")).click();

        $("[data-test-id='city'] input").doubleClick().sendKeys(Keys.BACK_SPACE);
        $("[data-test-id='city'] input").setValue("Як");
        $$(".menu-item .menu-item__control").find(exactText("Якутск")).click();

    }

    @Test
    void shouldTestDateNextWeek() {
        open("http://localhost:9999");
